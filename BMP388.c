...
#include "BMP388.h"
I2C_HandleTypeDef sensorAddress;
...
{
  uint8_t u8Ret;
  HAL_I2C_Mem_Read(&sensorAddress, DevAddr, RegAddr, I2C_MEMADD_SIZE_8BIT, &u8Ret, 1, 0x10);
  return u8Ret;
}

void I2C_WriteOneByte(uint8_t DevAddr, uint8_t RegAddr, uint8_t value)
{
  HAL_I2C_Mem_Write(&sensorAddress, DevAddr, RegAddr, I2C_MEMADD_SIZE_8BIT, &value, 1, 0x10);
  return;
}
/******************************************************************************
...
void pressSensorInit(PRESS_EN_SENSOR_TYPY *penPressSensorTyep, I2C_HandleTypeDef i2cAddrs)
{
	sensorAddress = i2cAddrs;
  bool bRet = false;
